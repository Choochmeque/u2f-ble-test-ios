...
class ViewController: UIViewController {
    ...
    @IBAction func sendRegister() {
        ...
        let challengeData = Data(buffer: UnsafeBufferPointer(start:&challenge, count: 32))
        let applicationParameterData = Data(buffer: UnsafeBufferPointer(start:&applicationParameter, count: 32))
    ...
    fileprivate func handleReceivedAPDU(_ manager: BluetoothManager, data: Data) {
        if let success = currentAPDU?.parseResponse(data), success {
            appendLogMessage("Successfully parsed APDU response of kind \(String(describing: currentAPDU))")
            if currentAPDU is RegisterAPDU {
                registerAPDU = currentAPDU as? RegisterAPDU
            }
        }
        else {
            appendLogMessage("Failed to parse APDU response of kind \(type(of: currentAPDU))")
        }
        currentAPDU = nil
    }
